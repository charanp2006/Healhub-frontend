    ...
    <div className='flex justify-between items-center text-sm py-4 mb-5 border-b border-b-border-light'>
        ...
        <ul className='md:flex items-start gap-5 font-medium hidden'>
            ...
            <NavLink to='/hospitals'>
                <li className='py-1'>Hospitals & Clinics</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden' />
            </NavLink>
            <NavLink to='/blogs'>
        ...
        <div className='flex items-center gap-4 '>
            ...
            ? <div onClick={()=>setDropdown(prev=>!prev)} className='flex items-center gap-2 cursor-pointer relative'>
                <img className='w-8 rounded-full' src={userData.image ? userData.image : assets.upload_icon} alt="" />
                <img className='w-2.5' src={assets.dropdown_icon} alt="" />
                <div className={`absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 ${dropdown ? 'block' : 'hidden'}`}>
            ...
            <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                ...
                <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                    ...
                    <NavLink onClick={()=>setShowMenu(false)} to='/hospitals'><p className='px-4 py-2 rounded inline-block'>Hospitals & Clinics</p></NavLink>
                    <NavLink onClick={()=>setShowMenu(false)} to='/blogs'><p className='px-4 py-2 rounded inline-block'>Blog</p></NavLink>
                    <NavLink onClick={()=>setShowMenu(false)} to='/demo'><p className='px-4 py-2 rounded inline-block'>Demo</p></NavLink>
