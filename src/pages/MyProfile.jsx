...
const MyProfile = () => {
  ...
  const updateUserProfileData = async () => {
    ...
    if (!userData.name || !userData.phone || !userData.address?.line1 || !userData.gender || !userData.dob) {
  ...
  return (
    userData && (
      <div className="max-w-lg flex flex-col gap-2 text-sm pt-5">

        {
          ...
          <label htmlFor="image" >
            <div className="inline-block relative cursor-pointer">
              <img className="w-36 rounded-full border-1 border-border-light" src={image ? URL.createObjectURL(image) : userData.image} alt="" />
              <img className="w-10 absolute bottom-12 right-12" src={image ? '' : assets.upload_icon } alt="" />
            </div>
            <input onChange={ (e) => setImage(e.target.files[0]) }type="file" id="image" hidden />
          </label>
          : <img className="w-36 rounded-full border-1 border-border-light" src={userData.image} alt="" />
        }
        ...
        <div>
          <p className="text-text-secondaryLight underline mt-3">CONTACT INFORMATION</p>
        ...
        <div>
          <p className="text-text-secondaryLight underline mt-3">BASIC INFORMATION</p>
